...
def getFormattedTextFast(bitmap, xDst, yDst, widthDst, heightDst, text, lineheight, newlineBreaks = true, explicitBreaksOnly = false)
  ...
  if defaultfontname.is_a?(Array)
    defaultfontname = defaultfontname.find { |i| Font.exist?(i) } || "Gill Sans MT Pro"
  elsif !Font.exist?(defaultfontname)
    defaultfontname = "Gill Sans MT Pro"
  end
...
<fn=X> ... </fn> - Formats the text in the specified font, or Gill Sans MT Pro
...
def getFormattedText(
  ...
  lineheight = bitmap.text_size("X").height + 2

  textchunks = []
  controls = []
  images = []
  oldtext = text
  while text[/<(\/?)(font|font color)(\s*\=\s*([^>]*))?>/]
    fix = ""
    if $~[4]
      color = $~[4]
      color = color[1...-1] if ["\"", "'"].any? { |s| color.start_with?(s) }
      color = color[1...] if color.start_with?("#")
      fix = "<c=" + color + ">"
    else
      fix = "</c>"
    end
    text = $~.pre_match + fix + $~.post_match
  end
  while text[/<(\/?)(lol-uikit-tooltipped-keyword)( key|)(\s*\=\s*([^>]*))?>/]
    fix = nil
    if $~[5]
      color = $~[5]
      case color
        when "'LinkTooltip_Description_Immobilize'" then fix = "<c=AD76C4>"
        when "'LinkTooltip_Description_AdaptiveDmg'" then fix = "<c=8155DE>"
      end
  ...
  while text[FORMATREGEXP]
    if ["icon", "img"].include?($~[2].downcase)
      ...
      if $~[3]
        controls.push([$~[2], $~[4], -1, $~[1] == "/" ? true : false])
      else
        controls.push([$~[2], "", -1, $~[1] == "/" ? true : false])
      end
      text = $~.post_match
    end
  end
  ...
  for i in 0...controls.length
    ...
    controls[i][0].downcase! if !FORMATTING.key?(controls[i][0].to_sym)
  end
  text = textchunks.join("")
  textchars = text.scan(/./m)
  colorstack = []
  ...
  while position < textchars.length
    ...
    for i in 0...controls.length
      if controls[i] && controls[i][2] == position
        ...
        if control == "c"
        ...
        elsif FORMATTING[control.to_sym]
          fmt = FORMATTING[control.to_sym]
          if endtag
            colorstack.pop if fmt[:color]
            alignstack.pop if fmt[:textalign]
          else
            base = rgbToColor(fmt[:color]) if fmt[:color]
            colorstack.push([base, nil]) if fmt[:color]
            alignstack.push(1) if fmt[:textalign]&.dig(:right)
          end
          boldcount += (endtag ? -1 : 1) if fmt[:bold]
          italiccount += (endtag ? -1 : 1) if fmt[:italic]
          underlinecount += (endtag ? -1 : 1) if fmt[:underline]
        elsif control == "gr"
          if endtag
            gradientstack.pop
          else
          ...
          if endtag
            ...
            fontnamestack.push(Font.exist?(fontname) ? fontname : "Gill Sans MT Pro")
          end
        ...
        elsif control == "br" || control == "li" # Line break
          if !endtag
            nextline += 1
          end
      ...
      if !endtag
        case type
          when "icon"
            ...
            graphic = "Graphics/Icons/iconDefault" if !resolveBitmap(graphic)
          when "img"
            param = param.sub(/\s+$/, "")
            param = param.split("|")
            graphic = param[0]
    ...
    if graphic
      begin
        ...
        xStart = 0 # 4
        fontHeight = bitmap.text_size("X").height # Icon should be center-aligned to font and font height is different for different fonts
