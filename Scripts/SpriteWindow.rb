module MessageConfig
  ...
  def self.defaultSystemFontName
    return MessageConfig.tryFonts(MessageConfig::FontName, "Gill Sans MT Pro", "Gill Sans MT Pro")
  end

  def self.defaultWindowskin
    skin = load_data("Data/System.rxdata").windowskin_name rescue nil
    if skin && skin != ""
      skin = resolveBitmap("Graphics/Windowskins/" + skin) || ""
    end
    if !skin || skin == ""
      skin = resolveBitmap("Graphics/System/Window")
    end
    if !skin || skin == ""
      skin = resolveBitmap("Graphics/Windowskins/001-Blue01")
    end
    return skin || ""
  end

  def self.getSpeechFrame
    if !@@defaultTextSkin
      skin = MessageConfig.defaultSpeechFrame()
      if !skin || skin == ""
        skin = MessageConfig.defaultWindowskin()
      end
  ...
  def self.getSystemFontName
    if !@@systemFont
      @@systemFont = defaultSystemFontName()
    end
    return @@systemFont
  end

  def self.getSystemFrame
    if !@@systemFrame
  ...
  def self.setSystemFrame(value)
    @@systemFrame = resolveBitmap(value) || ""
  end

  def self.setSpeechFrame(value)
    @@defaultTextSkin = resolveBitmap(value) || ""
  end

  def self.setSystemFontName(value)
    @@systemFont = MessageConfig.tryFonts(value, "Gill Sans MT Pro", "Gill Sans MT Pro")
  end

  def self.setTextSpeed(value)
    @@textSpeed = value - 6
  end
end
...
class GifBitmap
  ...
  def initialize(file, hue = 0)
    ...
    if bitmap
      ...
      # filestring=getFileString(filestrName) if filestring
...
def drawShadow(bitmap, x, y, width, height, string)
  return if !bitmap || !string

  drawShadowText(bitmap, x, y, width, height, string, nil, bitmap.font.color)
end

def drawShadowText(bitmap, x, y, width, height, string, baseColor, shadowColor = nil, align = 0)
  return if !bitmap || !string
...
def drawOutlineText(bitmap, x, y, width, height, string, baseColor, shadowColor = nil, align = 0)
  return if !bitmap || !string
...
def copyBitmap(dstbm, srcbm, x, y, opacity = 255)
...
def bottomLeftLines(window, lines, width = nil)
  window.x = 0
...
def isCurrentWindowskinDark
  testwindow = SpriteWindow_Base.new(0, 0, 1, 1)
  testwindow.setSkin(MessageConfig.getSpeechFrame())
  darkSkin = isDarkWindowskin(testwindow.windowskin)
  testwindow.dispose
  return darkSkin
end
...
def getDefaultTextColors(windowskin)
  if !windowskin || windowskin.disposed? ||
    ...
    colors = isDarkWindowskin(windowskin) ? ColorArrays : LightColorArrays
...
def updateSpriteHash(windows)
  for i in windows
    window = i[1]
    if window
      if window.is_a?(Sprite) || window.is_a?(Window)
        window.update if !raw_disposed?(window)
...
#   sprites[id].dispose
def disposeSprite(sprites, id)
  sprite = sprites[id]
  if sprite && !disposed?(sprite)
    sprite.dispose
  end
  sprites[id] = nil
end
...
def drawTextPositions(bitmap, textpos)
  for i in textpos
    ...
    x = i[1]
    y = i[2]
    if i[3] == true || i[3] == 1 || i[3] == :right # right align
      x -= textsize.width
    elsif i[3] == 2 || i[3] == :center  # centered
      x -= (textsize.width / 2).to_grid
    end
...
def drawImagePositions(bitmap, textpos)
  for i in textpos
    srcbitmap = AnimatedBitmap.new(bitmapName(i[0]))
    x = i[1]
    y = i[2]
    srcx = i[3]
    srcy = i[4]
...
def drawImagePositions2(bitmap, textpos, blend = nil)
  for i in textpos
    srcbitmap = AnimatedBitmap.new(bitmapName(i[0]))
...
def drawBitmap(bitmap, info)
  srcbitmap = info[0]
  x = info[1]
  y = info[2]
  srcx = info[3] || 0
  srcy = info[4] || 0
  width = info[5] || srcbitmap.width
  height = info[6] || srcbitmap.height
  srcrect = Rect.new(srcx, srcy, width, height)
...
def fadeOutAndHide(sprites)
  visiblesprites = {}
  deactivateWindows(sprites) {
    for j in 0..17
      setSpritesToColor(sprites, Color.new(0, 0, 0, j * 15))
      block_given? ? yield : updateSpriteHash(sprites)
    end
  }
  for i in sprites
    next if !i[1]
    next if raw_disposed?(i[1])
...
def fadeInAndShow(sprites, visiblesprites = nil)
  if visiblesprites
  ...
  deactivateWindows(sprites) {
    for j in 0..17
      setSpritesToColor(sprites, Color.new(0, 0, 0, ((17 - j) * 15)))
      block_given? ? yield : updateSpriteHash(sprites)
    end
  }
end
...
def restoreActivations(sprites, activeStatuses)
  return if !sprites || !activeStatuses

  for k in activeStatuses.keys
    if sprites[k] && sprites[k].is_a?(Window) && !disposed?(sprites[k])
...
def deactivateWindows(sprites)
  if block_given?
    activateWindow(sprites, nil) { yield }
  else
    activateWindow(sprites, nil)
  end
end
...
def activateWindow(sprites, key)
  return if !sprites

  activeStatuses = {}
  for i in sprites
    if i[1] && i[1].is_a?(Window) && !disposed?(i[1])
...
def alphaBlend(dstColor, srcColor)
...
def setSpritesToColor(sprites, color)
  return if !sprites || !color

  colors = {}
  for i in sprites
    next if !i[1] || raw_disposed?(i[1])
...
def addBackgroundPlane(sprites, planename, background, viewport = nil)
  ...
  bitmapName = resolveBitmap("Graphics/Pictures/#{background}")
  if bitmapName == nil
...
def setSystemFont(bitmap)
  fontname = MessageConfig.getSystemFontName()
  bitmap.font.name = fontname
  bitmap.font.size = getSystemFontSize(fontname)
end

def getSystemFontSize(fontname)
  return 24
end

# Gets the name of the system small font.
def smallFontName()
  return MessageConfig.tryFonts("Gill Sans MT Pro", "Gill Sans MT Pro")
end
...
def setSmallFont(bitmap)
  name = smallFontName()
  bitmap.font.name = name
  bitmap.font.size = getSystemFontSize(name)
end

# Sets a bitmap's font to the system narrow font.
def setNarrowFont(bitmap)
  name = NarrowFontName()
  bitmap.font.name = name
  bitmap.font.size = getSystemFontSize(name)
end
...
def rgssExists?(filename)
  filename = canonicalize(filename)
  if (safeExists?("./Game.rgssad") || safeExists?("./Game.rgss2a"))
    return getFileChar(filename) != nil
  else
...
class SpriteWindow_Base < SpriteWindow
  ...
  def initialize(x, y, width, height, viewport = nil)
    ...
    setSystemFont(self.contents) if self.contents
  end
...
##########################
# Bitmap drawing patches #
##########################
module BitmapTextPatch
  # args[1] = y
  # args[3] = height
  # args[4] = text
  def draw_text(*args)
    args[3] = self.text_size(args[4]).height # setSystemFont
    args[1] += 1 if args[3].odd?             # setSmallFont
    super(*args)
  end
end

class Bitmap
  prepend BitmapTextPatch
end
...
class Integer
  def to_grid
    return self.odd? ? self - 1 : self
  end
end

class Float # Just in case we end up with a Float instead of an Integer at some point
  def to_grid
    return self.to_i.to_grid
  end
end
