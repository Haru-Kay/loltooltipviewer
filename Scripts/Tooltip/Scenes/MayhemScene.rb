class MayhemScene
  def initialize
    ...
    @tabsel = 0
    @elements[:txtSearch] = SearchBar.new(0, @elements[:btn1].height)
    ...
    @augmentList = $cache.augments.keys.sort_by { |s| $cache.augments[s].name }
    @augments = []
    @loaded = false
  ...
  def searchAugments(text)
    @curSearch = text
    return $cache.augments.keys.sort_by { |s| $cache.augments[s].name } if text == ""

    ret = $cache.augments.keys.filter { |s|
      $cache.augments[s].name.downcase.include?(@curSearch) || toUnformattedText($cache.augments[s].tooltip).downcase.include?(@curSearch)
    }.sort_by { |s| $cache.augments[s].name }
